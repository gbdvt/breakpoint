...
export default function WorkSessionItem({ session }: Props) {
  return (
    ...
    >
      <div className="flex size-10 shrink-0 flex-col items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.04] text-[10px] font-semibold leading-tight tabular-nums text-white/70">
        <span className="text-white/90">{session.durationMin}</span>
        <span className="text-[8px] font-medium uppercase tracking-wide text-white/35">
          min
        </span>
      </div>
      ...
      <div className="flex shrink-0 flex-col items-end gap-0.5">
        ...
        <span className="text-[10px] font-semibold tabular-nums tracking-tight text-emerald-300/75">
